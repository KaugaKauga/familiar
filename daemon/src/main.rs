...
/// Familiar — your autonomous companion. Bind it to an issue, wake to a PR.
#[derive(Parser, Debug)]
...
enum Commands {
    /// Summon the familiar — start the daemon with a live interactive dashboard.
    Start {
...
fn run_status(runs_dir: &str) -> Result<()> {
    ...
    if !db_path.exists() {
        ...
        println!("Summon one first with: familiar start <owner/repo>");
        return Ok(());
    }
